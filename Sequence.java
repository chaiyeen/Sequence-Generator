    ...
    */
    public class Sequence implements Iterable<Integer> {
  	// set of constants that represent the sequence types supported by this sequence generator
	    ...
	    private Iterator<Integer> sequenceIterator; // iterator to iterate over this sequence // name of a "runner"
	...
	public Sequence(int[] command) {
		...
		case GEOMETRIC://determines the next number by multiplying the previous value by a fixed constant 
			// an iterator over an geometric sequence with initial element command[1], 
			// common ratio sequence size command[2]
			sequenceIterator = new GeometricSequenceGenerator(command[1], command[2], command[3]);
			break;

		case FIBONACCI://determines the next number by summing the two most recent previous values. 
			// To start the sequence, the first two values are conventionally 0 and 1
			// an iterator over a fibonacci sequence with sequence size command[1]
			sequenceIterator = new FibonacciSequenceGenerator(command[1]);
			break;

		case DIGIT_PRODUCT: // determines the next number by taking every NON ZERO digit of the previous number, 
			// multiply them and add the product to the previous number itself. 
			// iterator over a digit product sequence with initial number command[1] and sequence size command[2]
